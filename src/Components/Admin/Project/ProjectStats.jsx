...
import { fetchProjectStats } from "../../../Redux/Faculty/ProjectSlice"; 
...
const ProjectStatsCards = () => {
  ...
  const { stats, loading, error } = useSelector((state) => state.project);
  ...
  const renderStats = () => {
    ...
    const requiredKeys = ['totalProjects', 'completedProjects', 'ongoingProjects', 'fundedProjects'];
  ...
  if (error) {
    return (
      <div className="text-red-500 text-center font-medium p-4">
        Failed to load project stats: {error}
      </div>
    );
  }
