...
import { fetchLectureStats } from "../../../Redux/Faculty/LectureSlice";
...
const LectureStatsCards = () => {
  const dispatch = useDispatch();
  const { stats, isLoading, error } = useSelector((state) => state.lecture);

  useEffect(() => {
    dispatch(fetchLectureStats());
  }, [dispatch]);
  ...
  if (error) {
    return (
      <div className="text-red-500 text-center font-medium p-4">
        Failed to load lecture stats: {error}
      </div>
    );
  }
...
export default LectureStatsCards;
